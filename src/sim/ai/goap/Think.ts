...
export class Think<Owner, World> extends CompositeGoal<Owner> {
   ...
   * we have no current subgoal), then ticks the active subgoal.
   */
  override tick(ctx: GoalContext): GoalStatus {
    return this.executeSubgoals(ctx);
  }
  ...
  step(world: World, ctx: GoalContext): GoalStatus {
    ...
    // Top-level Think never "completes" — when its subgoal completes
    // we just re-arbitrate next tick.
